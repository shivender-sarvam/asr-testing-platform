...
def show_testing_interface():
    ...
    if st.session_state.current_test_index < len(st.session_state.test_data):
        ...
        """.format(recording_key=recording_key)
            ...
            # All form submission code has been removed - we only use file_uploader now
        ...
        # Define audio_submit_key outside so it's available everywhere
        audio_submit_key = st.query_params.get('audio_submit')
        
        if audio_bytes is None:
            # SOLUTION 2: Two-step rerun process (fallback for direct recording)
        ...
        # Check for audio_loaded param (fallback for direct recording - only if file_uploader didn't provide audio)
        if audio_bytes is None:
                ...
                try:
                    ...
                    if uploaded_audio.name.endswith('.webm'):
                        ...
                        if not conversion_success:
                            ...
                            st.error("❌ **BLOCKING ERROR: Cannot proceed without WAV conversion. Please check dependencies.**")
                            st.stop()
